...
即使分布式运行时还没有共享文件系统，原生输入框也要求会话归属于 Workspace。因此 PostgreSQL 负责租户级逻辑 Workspace 记录，以及会话归属与顺序。注册会创建默认 `/workspace` 记录，迁移则把全部已有会话挂到各租户默认工作区。Workspace RPC 与 host-stream 增量实现跨副本原生客户端契约。当前 path 只是逻辑标签；文件系统物化会等部署方选定共享卷、仓库签出或对象存储工作区策略后再实现。
