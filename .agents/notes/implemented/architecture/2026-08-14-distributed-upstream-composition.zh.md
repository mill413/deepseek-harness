...
本说明中通过明确失败暂缓运行时能力的阶段性决策，已由[单例 Workspace 中的上游运行时一致性](2026-08-14-distributed-runtime-parity.md)取代。本说明继续负责浏览器组合发现与租户感知 Web 协议；后续说明负责 Agent、工具、交互、subagent、工作流、Cordis 和预设执行。
...
**在每个可见上游插件拥有执行所有者前就宣称支持。** 否决，因为客户端代码并不拥有执行语义。后续运行时一致性决策通过增加缺失所有者来解决问题，而没有削弱该要求。
...
- 服务端能力仍必须有明确的租户感知所有者；运行时一致性决策为正式预设与浏览器界面提供了这些所有者。
